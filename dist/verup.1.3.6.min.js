#!/usr/bin/env node
var path=require("path"),fs=require("fs"),ver_reg=[/^((?:\$|(?:\s*\**\s*@)|(?:\s*(?:var|,)?\s+))version[\s\:='"]+)([0-9]+(?:\.[0-9]+){2,2})/i,/^(\s*const\s+VERSION[\s='"]+)([0-9]+(?:\.[0-9]+){2,2})/i,/^(\s?\*.*v)([0-9]+(?:\.[0-9]+){2,2})/],bump="1",name="",_a="b";process.argv.forEach(function(a,c){if(!(2>c))if("-"==a.slice(0,1)&&isNaN(parseFloat(a)))_a=a.slice(1);else{switch(_a){case "b":bump=a;break;case "n":name=a}_a="b"}});var packFile=findPackage(__dirname,name);
packFile||(console.log("package.json file not found"),process.exit(1));var _root=path.dirname(packFile),packo=require(packFile);packo||(console.error("Can't read package.json file"),process.exit(1));var _verup=packo.verup;_verup||(console.log("package.json doesn't have a `verup` property defined"),process.exit(1));var files=_verup.files;_verup.regs&&(ver_reg=_verup.regs.map(function(a){return new RegExp(a,"i")}));var over=packo.version;
